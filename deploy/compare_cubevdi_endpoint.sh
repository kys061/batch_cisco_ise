...
function compare_cubevdi_endpoint()
{
    echo -e "" |tr -d "\n" > $RESULT_PATH/creating_$2_lists.txt.tmp
    _groupId=$1
    webportal_mac=($(mysql $db_name -e "select mac_address from internal_assets_mac" |egrep '^([[:xdigit:]]{2}[:.-]?){5}[[:xdigit:]]{2}$'))
    if [ -f $RESULT_PATH/ise_$2_endpoint_lists.txt ]; then
      for ((j = 0; j < ${#webportal_mac[@]}; j++)); do
          ...
          if [ "$is_exist_in_ise" != "$webportal_mac_address" ]; then
            echo -e "Writing ise endpoint($webportal_mac_address).." | awk '{ print strftime("%Y-%m-%d %T") " '$CUBEVDI' ", $0; fflush() }' >> $LOG
      ...
      if [ "$is_exist_in_creating_lists" == "false" ]; then
        echo -e "There is no lists to write in creating lists.." | awk '{ print strftime("%Y-%m-%d %T") " '$CUBEVDI' ", $0; fflush() }' >> $LOG
      fi
      is_exist_in_creating_lists=false
    else
      echo -e "There is no ise_$2_endpoint_lists.txt,, please check!!" | awk '{ print strftime("%Y-%m-%d %T") " '$CUBEVDI' ", $0; fflush() }' >> $LOG
    fi
}
...
while true; do
  rotate_lists $RESULT_PATH/creating_${CUBEVDI}_lists.txt
  sleep 1
  compare_cubevdi_endpoint $CUBEVDI_ID $CUBEVDI
  rotate_log
  sleep 10
done